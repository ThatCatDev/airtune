...
// We also proactively send timing packets every 3 seconds.
...
// that communicates with the given remote address.
func NewTimingChannel(conn *net.UDPConn, remoteAddr *net.UDPAddr) *TimingChannel {
...
// and responds with timing response packets (PT 83).
func (t *TimingChannel) listenRequests(ctx context.Context) {
	...
	for {
		...
		// Extract the reference timestamp from the request (bytes 24-31)
		// This is the time the device sent the request
		refSecs := binary.BigEndian.Uint32(buf[24:28])
		refFrac := binary.BigEndian.Uint32(buf[28:32])

		// Build timing response (PT 83)
